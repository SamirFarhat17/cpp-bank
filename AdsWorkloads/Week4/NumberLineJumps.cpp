...
string kangaroo(int x1, int v1, int x2, int v2) {
    ...
    if(x1 == x2) return "YES";
    if((x1 > x2 && v1 >= v2) || (x1 < x2 && v2 >= v1)) return "NO"; 
    
    // equation is x1 + v1*t = x2 + v2*t // just solve for t
    // x1 - x2 = t(v2 - v1) => [(x1-x2)/(v2-v1)] = t
    
    
    int diff = x1 - x2;
    int denom = v2 - v1;
    float t = (float)diff/denom;

    if(t >= 1 && (diff % denom == 0)) return "YES";
