...
void Bank::executeTransaction(Transaction transact) {
    try {
        ...
        if (sourceIt == mappings.end() || destIt == mappings.end()) 
            throw TransactionException("One or more of Accounts not found");

        if (transact.amount > sourceIt->second->getBalance()) 
            throw TransactionException("Payment greater than account balance");
